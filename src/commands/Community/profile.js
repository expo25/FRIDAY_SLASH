    ...
    async execute(Interaction, client) {
        ...
        const profileEmbed = new MessageEmbed()
            ...
            .addFields(
                ...
                { name: '\u200b', value: `[Link to User Avatar](${user.displayAvatarURL({ dynamic: true, size: 512 })})`, inline: true },)
        ...
        if (user.flags ==  4194304) badges.push("<:nitro:1065843259869499393>");
        ...
        profileEmbed.addFields({ name: 'Profile Badges', value: badges.join(', '), inline: true })

        if (data.banner) {
            let url = data.banner.startsWith("a_") ? ".gif?size=4096" : ".png?size=4096";
            url = `https://cdn.discordapp.com/banners/${user.id}/${data.banner}${url}`;
            profileEmbed.addFields({ name: '\u200b', value: `[Link to Profile Banner](${url})` })
